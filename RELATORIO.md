...
- **Isolamento da Chave de IA (Serverless / Express)**: O Front-End React não gerencia mais o *Gemini 1.5 Flash*. Passamos do `@google/genai` pro pacote nativo `@google/generative-ai` estruturado fora da pasta *src*, e rodamos a api através de Scripts de Monitoramento simultâneos (TSX). O Vercel agora tem caminho aberto para rodar a IA sob a sombra dos Serverless Functions impedindo que atacantes no Console Front-end detectem ou burlem a API_KEY do desenvolvedor.
